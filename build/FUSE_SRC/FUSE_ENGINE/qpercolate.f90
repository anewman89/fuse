...
SELECT CASE(SMODL%iQPERC)
 ...
 CASE(iopt_perc_lower) ! perc defined by moisture content in lower layer (SAC)
  ...
  !print *, 'terms ', TSTATE%TENS_2/DPARAM%MAXTENS_2,TSTATE%FREE_2/(DPARAM%MAXFREE_2A+DPARAM%MAXFREE_2B),MPARAM%SACPMLT, TSTATE%WATR_2/MPARAM%MAXWATR_2, MPARAM%SACPEXP
  IF(TSTATE%WATR_2/MPARAM%MAXWATR_2 .LT. 1._SP) THEN
